...
function generate_option(this_account) {
	...
	this_element.style.whiteSpace = "nowrap";

	var userhead = document.createElement("img");
	userhead.src = this_account.site_info.userhead;
	userhead.width = 17;
	userhead.height = 17;
	userhead.style.verticalAlign = "middle";
	var username = document.createTextNode(this_account.username);
	this_element.appendChild(userhead);
	this_element.appendChild(username);
...
window.onload = function() {
	...
	chrome.runtime.sendMessage({"command":"localStorage","mode":"get","key":"lj_juggler_accounts"}, function (response) {
		...
		if(kill_me && kill_me.length > 0 && kill_me.length < 2) {
