...
function getLJchallenge(interface_url, callback) {
	...
	conn.onreadystatechange = function() {
		if (conn.readyState == 4 && conn.status == 200) {
			getLJchallenge_callback(callback, conn);
		}
	};
	conn.send(params);
}
...
function loginas(this_account, callback) {
	...
	if (site_info.name == "Dreamwidth") {
		...
		conn.onreadystatechange = function() {
			if (conn.readyState == 4 && conn.status == 200) {
				loginas_callback(this_account, callback, conn);
			}
		};
		conn.send(params);
	} else {
...
function logout_this_cookie(cookie, callback) {
	if(cookie != undefined) {
		...
		try {
			...
			conn.onreadystatechange = function() {
				if (conn.readyState == 4 && conn.status == 200) {
					logout_this_cookie_callback(cookie, callback, conn);
				}
			};
			conn.send(params);
		} catch (e) {
...
function update_account(change_me) {
	...
	if (stored_account_data) {
		account_list = JSON.parse(stored_account_data);
	}
	localStorage["lj_juggler_accounts"] = JSON.stringify(account_list);
}
...
function update_check() {
	var current_version = getVersion(function (current_version) {
		...
		if(old_version != current_version) {
			...
			console.log("Data structures up to date");
		}
	});
}
...
function getVersion(callback) {
	...
	conn.onreadystatechange = function() {
		if (conn.readyState == 4 && conn.status == 200 && callback) {
			getVersion_callback(callback, conn);
		}
	};
	conn.send(null);
}
...
function get_config_site_info(site_info) {
	return LJlogin_sites[LJlogin_keys.indexOf(site_info.name)];
}
